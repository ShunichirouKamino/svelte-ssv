<script lang="ts">
	...
	import { loginSchemaArktype, type LoginDataArktype } from "../lib/schemas/login-arktype";

	let form: Form<LoginDataArktype> = $state(
		createForm(loginSchemaArktype, { email: "", password: "" }),
	);
	...
	function handleSubmit(e: SubmitEvent) {
		...
		console.log("Login (ArkType):", result.data);
		submitted = true;
		form.reset();
	}
</script>
...
<h1>Login — ArkType</h1>
<p class="description">
	Same ssv API, but the schema is defined with <strong>ArkType</strong> instead of Zod.
	Works via <a href="https://github.com/standard-schema/standard-schema">Standard Schema V1</a>.
</p>

<div class="lib-badge">Schema: <code>arktype</code></div>
...
<style>
	...
	.lib-badge { display: inline-block; font-size: 0.75rem; font-weight: 600; background: #818cf8; color: white; padding: 0.15rem 0.6rem; border-radius: 4px; margin-bottom: 1rem; }
